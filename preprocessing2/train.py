...
def per_class_acc(logits, labels):
    ...
    for c in range(3):
        m      = (labels == c)
        res[names[c]] = (preds[m] == labels[m]).float().mean().item() \
                        if m.sum() > 0 else 0.0
    return res
...
def train(args):
    ...
    # Cosine scheduler — плавно уменьшает lr от args.lr до 1e-6
    # Лучше чем ReduceLROnPlateau для нашей задачи
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"{epoch:>6} | {tr_loss:>10.4f} | {tr_acc:>8.1%} | "
              f"{vl_loss:>8.4f} | {vl_acc:>6.1%} | {lr_now:.1e}")
    ...
    with open(os.path.join(save_dir, f'{args.model}_metrics.pkl'), 'wb') as f:
        pickle.dump({'model': args.model, 'best_val': best_val,
                     'test_acc': ts_acc, 'test_pc': ts_pc,
                     'history': dict(history)}, f)
...
def parse_args():
    ...
    p.add_argument('--pkl_path',   default='/content/drive/MyDrive/Diploma2/mosei_v3.pkl')
